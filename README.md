...
5. В последнем запросе "название альбомов, содержащих наименьшее количество треков" понадобилась конструкция WITH alias AS (SELECT ...), иначе приходилось дважджы внутри этого запроса выполнять вложенный запрос.
